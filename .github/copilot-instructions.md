...
- `quizStore` loads `q1…q38` from active locale via `i18n`, throwing immediately if `questions.qN` is missing so localization drift is caught early.
...
- **quizStore** owns navigation, `canProceed`, watch-based locale reloads; push quiz logic here, not views. Hardcoded `questionIds` array (`q1…q38`) must match every translation file—missing entries crash on locale switch.
...
- **Adding questions**: Edit all 4 locale files (`src/data/translations/{en,af,xh,zu}.json`), append id to `quizStore.questionIds`, create scoring rows (weight + 14 party scores) in appropriate topical JSON under `src/data/scoring/`, maintain ordering in `scoring/index.ts`. The current quiz uses 38 questions (`q1…q38`).
